                    ...
                    <div class="py-2">
                        <MenuItem @click.stop.prevent="fileStore.clearCacheForFile(logFile)" v-slot="{ active }">
                            <button :class="[active ? 'active' : '']">
                                <CircleStackIcon
                                    v-show="!fileStore.clearingCache[logFile.identifier]"
                                    class="h-4 w-4 mr-2"
                                />
                                <SpinnerIcon v-show="fileStore.clearingCache[logFile.identifier]" />
                                <span
                                    v-show="
                                        !fileStore.cacheRecentlyCleared[logFile.identifier] &&
                                        !fileStore.clearingCache[logFile.identifier]
                                    "
                                    >Clear index</span
                                >
                                <span
                                    ...
                                    >Clearing...</span
                                >
                                <span v-show="fileStore.cacheRecentlyCleared[logFile.identifier]" class="text-brand-500"
                                    >Index cleared</span
                                >
                            </button>
                        </MenuItem>
...
const confirmDeletion = async () => {
    if (
        confirm(`Are you sure you want to delete the log file '${props.logFile.name}'? THIS ACTION CANNOT BE UNDONE.`)
    ) {
