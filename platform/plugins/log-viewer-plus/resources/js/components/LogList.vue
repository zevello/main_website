    ...
    <div class="h-full w-full py-5 log-list">
        <div class="flex flex-col h-full w-full md:mx-3 mb-4">
            ...
            <div v-if="displayLogs" class="relative overflow-hidden h-full text-sm">
                <!-- pagination settings -->
                <div
                    class="mx-2 mt-1 mb-2 text-right lg:mx-0 lg:mt-0 lg:mb-0 lg:absolute lg:top-2 lg:right-6 z-20 text-sm text-gray-500 dark:text-gray-400"
                >
                    ...
                    <div class="inline-block min-w-full max-w-full align-middle">
                        <table class="table-fixed min-w-full max-w-full border-separate" style="border-spacing: 0">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th scope="col" class="w-[120px] hidden lg:table-cell">
                                        <div class="pl-2">Level</div>
                                    </th>
                                    ...
                                    >
                                        <td class="log-level truncate">
                                            <div class="flex items-center lg:pl-2">
                                                ...
                                                >
                                                    <span class="sr-only" v-if="!logViewerStore.isOpen(index)"
                                                        >Expand log entry</span
                                                    >
                                                    <span class="sr-only" v-if="logViewerStore.isOpen(index)"
                                                        >Collapse log entry</span
                                                    >
                                                    <span class="w-full h-full group-hover:hidden group-focus:hidden">
                                                        <ExclamationCircleIcon v-if="log.level_class === 'danger'" />
                                                        <ExclamationTriangleIcon
                                                            v-else-if="log.level_class === 'warning'"
                                                        />
                                                        <InformationCircleIcon v-else />
                                                    </span>
                                                    <span
                                                        class="w-full h-full hidden group-hover:inline-block group-focus:inline-block"
                                                    >
                                                        <ChevronRightIcon
                                                            :class="[
                                        ...
                                        <td
                                            class="whitespace-nowrap text-gray-500 dark:text-gray-300 dark:opacity-90 hidden lg:table-cell"
                                            v-html="highlightSearchResult(log.environment, searchStore.query)"
                                        ></td>
                                        <td
                                            class="max-w-[1px] w-full truncate text-gray-500 dark:text-gray-300 dark:opacity-90"
                                            v-html="highlightSearchResult(log.text, searchStore.query)"
                                        ></td>
                                        <td
                                            class="whitespace-nowrap text-gray-500 dark:text-gray-300 dark:opacity-90 text-xs hidden lg:table-cell"
                                        >
                                    ...
                                    <tr v-show="logViewerStore.isOpen(index)">
                                        <td colspan="6">
                                            <div class="lg:hidden flex justify-between px-2 pt-2 pb-1 text-xs">
                                                <div class="flex-1">
                                                    <span class="font-semibold">Time:</span> {{ log.datetime }}
                                                </div>
                                                <div class="flex-1">
                                                    <span class="font-semibold">Env:</span> {{ log.environment }}
                                                </div>
                                                <div>
                                            ...
                                            <template v-if="log.contexts && log.contexts.length > 0">
                                                <p
                                                    class="mx-2 lg:mx-8 pt-2 border-t font-semibold text-gray-700 dark:text-gray-400"
                                                >
                                                    Context:
                                                </p>
                                                <template v-for="context in log.contexts">
                                                    <pre
                                                        class="log-stack"
                                                        v-html="JSON.stringify(context, null, 2)"
                                                    ></pre>
                                                </template>
                                            </template>
                                            <div v-if="log.full_text_incomplete" class="py-4 px-8 text-gray-500 italic">
                                                ...
                                                {{ LogViewer.max_log_size_formatted }}. The full size of this log entry
                                                is <strong>{{ log.full_text_length_formatted }}</strong>
                                            </div>
                                        </td>
                                    </tr>
                                </tbody>
                                    ...
                                    <td colspan="6">
                                        <div class="bg-white text-gray-600 dark:bg-gray-800 dark:text-gray-200 p-12">
                                            ...
                                            <div class="text-center mt-6">
                                                ...
                                                <button
                                                    v-if="
                                                        severityStore.levelsFound.length > 0 &&
                                                        severityStore.levelsSelected.length === 0
                                                    "
                                                    class="px-3 ml-3 py-2 border dark:border-gray-700 text-gray-800 dark:text-gray-200 hover:border-brand-600 dark:hover:border-brand-700 rounded-md"
                ...
                <div
                    class="absolute inset-0 top-9 md:px-4 z-20"
