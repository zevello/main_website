<template>
    <div
        ...
        :class="[
            fileStore.sidebarOpen
                ? 'left-0 right-0 md:left-auto md:right-auto'
                : '-left-[200%] right-[200%] md:left-auto md:right-auto',
        ]"
    >
        <file-list></file-list>
    </div>
        ...
        <p class="text-xs text-gray-500 dark:text-gray-400 mr-5 -mb-0.5">
            <template v-if="logViewerStore.performance?.requestTime">
                <span>
                    ...
                    <span class="font-semibold">{{ logViewerStore.performance.memoryUsage }}</span>
                </span>
                <span class="mx-1.5">&middot</span>
                <span>
                    <span class="hidden md:inline">Duration: </span>
                    <span class="font-semibold">{{ logViewerStore.performance.requestTime }}</span>
                </span>
                <span class="mx-1.5">&middot</span>
            </template>
            <span>
                <span class="hidden md:inline">Version: </span>
                <span class="font-semibold">{{ LogViewer.version }}</span>
            </span>
        </p>
    </div>
...
onMounted(() => {
    ...
    window.onresize = () => logViewerStore.setViewportDimensions(window.innerWidth, window.innerHeight)
})
