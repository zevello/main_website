@if ($payment)
    ...
    @if ($refunds)
        ...
        <h6 class="alert-heading">
            {{ trans('plugins/payment::payment.refunds.title') . ' (' . count((array) $refunds) . ')' }}</h6>
        <hr class="m-0 mb-4">
        @foreach ($refunds as $item)
            ...
            >
                ...
                <p>{{ trans('plugins/payment::payment.refunds.create_time') }}:
                    {{ Carbon\Carbon::now()->parse($item->create_time) }}</p>
                <p>{{ trans('plugins/payment::payment.refunds.update_time') }}:
                    {{ Carbon\Carbon::now()->parse($item->update_time) }}</p>
            </div>
            <br />
        @endforeach
    @endif
